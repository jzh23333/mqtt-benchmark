...
import (
	...
	"github.com/krylovsk/mqtt-benchmark/mqtt/packets"
)
...
type incomingComms struct {
	...
	incomingPubAck *packets.PubackPacket
}
	...
	go func() {
		for {
			...
			case *packets.PublishPacket:
				...
				DEBUG.Println(NET, "startIncomingComms: received publish, msg:", m)
				output <- incomingComms{incomingPub: m}
			case *packets.PubackPacket:
				DEBUG.Println(NET, "startIncomingComms: received puback, id:", m.MessageID)
				DEBUG.Println(NET, "startIncomingComms: received puback, msg:", m)
				//output <- incomingComms{incomingPubAck: m}
				c.getToken(m.MessageID).flowComplete()
				c.freeID(m.MessageID)
